	...
	if(distant_source)
		for(var/mob/M as() in SSmobs.clients_by_zlevel[z])
			...
			if(dist_far <= long_range && dist_far > range)
				M.playsound_local(distant_source, "sound/effects/[selected_sound]_distance.ogg", 100, falloff_exponent = 20, mixer_channel = CHANNEL_SOUND_EFFECTS)
				...
				M.playsound_local(source, "sound/effects/[selected_sound].ogg", 100, falloff_exponent = range / 2, mixer_channel = CHANNEL_SOUND_EFFECTS)
