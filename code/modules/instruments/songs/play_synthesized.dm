...
/datum/song/proc/playkey_synth(key, mob/user)
	...
	for(var/i in hearing_mobs)
		...
		if(!(M?.client?.prefs?.toggles & SOUND_INSTRUMENTS))
			continue
		M.playsound_local(get_turf(parent), null, volume, FALSE, K.frequency, INSTRUMENT_DISTANCE_NO_FALLOFF, channel, null, copy, distance_multiplier = INSTRUMENT_DISTANCE_FALLOFF_BUFF, mixer_channel = mixing_channel)
